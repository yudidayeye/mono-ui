...
function createRgbaColor(r: number, g: number, b: number, a: number = 1): RGBAColor {
  return {
    ...
    get rgba() {
      return `rgba(${this.rgbTxt}, ${!Number.isNaN(this.args[3]) ? this.args[3] : 1})`;
    },
  };
}
...
function parseCssFunc(str: string): RGBAColor | null {
  ...
  if (func === 'rgb' || func === 'rgba') {
    const [r, g, b, a] = args;
    return createRgbaColor(r, g, b, !Number.isNaN(a) ? a : 1);
  }
...
export function mixRgbColor(source: RGBAColor, target: RGBAColor, percent: number): RGBAColor {
  ...
  const [rr, gg, bb] = res;
  return createRgbaColor(rr, gg, bb, !Number.isNaN(source.args[3]) ? source.args[3] : 1);
}
