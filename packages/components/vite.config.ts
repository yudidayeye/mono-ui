 ...
 * @LastEditTime: 2024-01-05 14:13:23
...
function pluginMoveStyles(mode: string): PluginOption {
  ...
  return {
    ...
    async closeBundle() {
      ...
      // 在待处理的子包中排除掉自己
      const uiIndex = packages.findIndex((pkg) => pkg === 'components');
      if (uiIndex > 0) {
...
// @ts-ignore
export default defineConfig(({ mode }) => generateVueConfig(
  {
    outDir: OUT_REL,
